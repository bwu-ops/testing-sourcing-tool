...
def sanitize_llm_output(text: str) -> str:
    ...
    Use this before rendering LLM-generated text in any HTML context.
    """
...
async def generate_text(
    *,
    ...
    The return value is UNTRUSTED external input. Callers MUST sanitize
            ...
            if response.status_code == 429 and retry_after:
                ...
                if wait_seconds > 60:
                    raise LLMProviderError(
                        f"{provider_name} rate limited with Retry-After {retry_after}s — too long to wait."
                    )
            else:
                wait_seconds = 0.5 * (2**attempt)
            logger.warning(
