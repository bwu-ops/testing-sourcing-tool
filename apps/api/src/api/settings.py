...
class AppSettings(BaseSettings):
    ...
    # SECURITY: controls debug mode, dev auth bypass, cookie security, and startup guards
    # Any value other than "local" or "test" is treated as production
    app_env: str = "local"
    ...
    # SECURITY: empty string means ALL email domains are accepted — always set in production
    ...
    def _check_production_secrets(self) -> AppSettings:
        if self.session_secret == "dev-only-change-me" and self.app_env not in {"local", "test"}:  # noqa: S105 - comparing against known default, not a real secret
            raise ValueError(
                "SESSION_SECRET must be changed from the default before running "
                'outside local/test. Generate one with: python -c "import secrets; print(secrets.token_urlsafe(48))"'
            )
        return self
