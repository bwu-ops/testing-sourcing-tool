...
# Replit currently pins Node 22, while local macOS defaults to .nvmrc.
REPLIT_NODE_MAJOR="22"
SUPPORTED_NODE_VERSION_TEXT="${REQUIRED_NODE_MAJOR}.x"
if [[ "$REPLIT_NODE_MAJOR" != "$REQUIRED_NODE_MAJOR" ]]; then
  SUPPORTED_NODE_VERSION_TEXT="${REPLIT_NODE_MAJOR}.x or ${SUPPORTED_NODE_VERSION_TEXT}"
fi
...
check_node_version() {
  ...
  if [[ "$detected_version" == "${REQUIRED_NODE_MAJOR}."* ]] || [[ "$detected_version" == "${REPLIT_NODE_MAJOR}."* ]]; then
    echo "[ok] node version ${detected_version}"
    return
  fi

  echo "[missing] node version ${detected_version}; expected ${SUPPORTED_NODE_VERSION_TEXT}"
